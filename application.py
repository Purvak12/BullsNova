...
application.config['SESSION_REDIS'] = redis.from_url('redis://default:0elOK3wuFk56XcIR0glg9Lq0p7x16n8o@redis-16366.c267.us-east-1-4.ec2.redns.redis-cloud.com:16366')
...
# application.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///'+ os.path.abspath(  os.path.join("src","finances.db")  )
...
# with application.app_context():
#     Bought.__table__.create(db.engine, checkfirst=True)
class Sold(db.Model):
    __tablename__ = "sold"
    id = db.Column(db.Integer, primary_key=True)
...
# with application.app_context():
#     Sold.__table__.create(db.engine, checkfirst=True)
...
class PortfolioSchema(ma.SQLAlchemyAutoSchema):
    class Meta:
        ...
        model = Portfolio
        load_instance = True
class BoughtSchema(ma.SQLAlchemyAutoSchema):
    class Meta:
        # fields = ('time', 'symbol', 'shares_bought', 'price_bought')
        model = Bought
        load_instance = True
class SoldSchema(ma.SQLAlchemyAutoSchema):
    class Meta:
